...
export default function GuidesPage() {
  const categories = [
    ...
    { icon: Terminal, title: "POS Operations", count: "24 articles" },
    { icon: Zap, title: "Automations", count: "8 articles" },
    { icon: Code, title: "Developer API", count: "Documentation" },
  ];
          ...
          <div className="lg:col-span-1 space-y-4 lg:sticky lg:top-32 h-fit">
            {categories.map((cat, i) => (
              <FadeIn key={i} delay={i * 0.1} direction="right">
                <button className="w-full text-left p-4 rounded-2xl hover:bg-muted/50 transition-colors flex items-center gap-4 group cursor-pointer border border-transparent hover:border-border/50">
             ...
             <FadeIn delay={0.3} direction="up">
                <div className="p-10 md:p-16 rounded-[3rem] bg-card/80 backdrop-blur-md border border-border/50 relative overflow-hidden shadow-2xl">
                  ...
                  <div className="space-y-4 relative z-10">
                    {[
                      "Setting up your first floor plan",
                      "Configuring complex happy hour modifiers",
                      "Syncing room charges to Opera PMS",
                      "Understanding ingredient batch depletion"
                    ].map((guide, i) => (
                      <div key={i} className="p-6 md:p-8 rounded-[1.5rem] border border-border/30 bg-background/50 hover:bg-background hover:border-primary/50 transition-all duration-300 cursor-pointer group flex items-center justify-between shadow-sm hover:shadow-[0_10px_30px_rgba(255,92,0,0.05)]">
                        <span className="text-xl font-medium group-hover:text-primary transition-colors">{guide}</span>
                        <div className="h-10 w-10 rounded-full border border-border flex items-center justify-center group-hover:bg-primary group-hover:border-primary group-hover:text-white transition-all duration-300">
                          →
                        </div>
                      </div>
                    ))}
